...
;;
;; srep - Emacs で Excel のオ?トフィル機能のようなことをする
;;
;; [インスト?ル方法]
;;
;;   ロ?ド可能なディレクトリに srep.el を置き、.emacs などで
...
;; [リピ?ト??の追加方法]
;;
;;   ?自にリピ?ト?? (アナライザとリピ?タ) を??して
;;   srep-repeat-functions に登?することで、繰り返し要素を
;;   追加することができます。
;;
;;   決まった?語を繰り返すだけであれば、マクロ srep-define-word-repeater を使って
;;   簡?にリピ?ト??を定義できます。
;;
;;   リピ?ト??を追加するには以下のようにします。
;;
...
;;   * リリ?ス 0.1.1
;;
;;   * srep-repeat-functions の構文エラ?を修正。
;;
;;   * YYYY-MM-DD 形式のリピ?ト??がオ?バ?フロ?で正常動作しないバグを修正。
;;
;; 2011-12-20  Morimoto, Ken  <ken.m.pp1@gmail.com>
;;
;;   * リリ?ス 0.1.0
...
(defvar srep-repeat-functions
  '((srep-ana:yyyy-mm-dd srep-rep:yyyy-mm-dd)
    ...
    (srep-ana:dayofweek-ko-short srep-rep:dayofweek-ko-short)    
...
;; リピ?ト回?をユ?ザに尋ねる
;; 結果を文字列化して返す。?理後に srep を終了する場合は最後に "!" が付いている。
(defun srep--read-times ()
  (let* ((input (read-string "How many times? (default 1, end with '!' to quit): "
    ...
    (when (or (<= num 0) (equal input "")) ; 終了指定とデフォルト指定は履?に?さない
...
(defun srep--split (str delimiter backward)
  (let ((elements (split-string str (regexp-quote delimiter))))
    ...
    (when (and (not (equal str "")) (equal delimiter ""))
      (setq elements (cdr elements))) ; 空文字列で?切ると必ず最初に余分な空文字列の要素が付くので削除
    (let ((element-num (length elements)))
      (when (equal (nth (1- element-num) elements) "")
	(setcdr (nthcdr (- element-num 2) elements) nil))) ; 選?範?の最後のデリミタは無視 (最後に空文字列の要素が付く)
    elements))

;; 繰り返しテ?ブルの作成
;;   inputs  入力文字列群のリスト
;;   => 繰り返しテ?ブル。srep--repeat で使用する。
;;        ((文字列 文字列 ...)
;;         (リピ?タ ?列?? (?列??のパラメ?タ) サンプル文字列)
;;         (文字列 文字列 ...)
;;         (リピ?タ ?列?? (?列??のパラメ?タ) サンプル文字列)
;;         :
;;         :)
;;      ?列??: 第 n 項の値を求める??。
;;      ?列??のパラメ?タ: 初項、公差、公比など。
;;      サンプル文字列: 繰り返し基文字列のサンプル。出力書式の?考にする。
;;
;; [例]
	...
	(while strs
	  (let ((str (car strs)) result)
		...
		(setq results (cons result results))
	      ...
	      (setq results nil))))
	(when results
	  (setq results (reverse results)) ; cons で追加していたので反?
	  ;; アナライザを全文字列に適用できた
...
;; 決まった?語を繰り返すだけのリピ?ト??を定義するマクロ
;;   name       名前
;;   word-list  ?語リスト
;;
;; このマクロによって、アナライザとリピ?タが定義される。
...
;; ?語リストの中から?語を探す (大文字小文字の?別なし)
;;   word       探したい?語
;;   word-list  ?語リスト
;;   => 見つかった場合は要素番?、見つからなかった場合は nil。
...
(dont-compile
  (when (fboundp 'expectations)
	...
	(srep--arithmetic-sequence-p '()))
      (expect '(srep--arithmetic-sequence-nth (1.0 0.0)) ; ?項
	...
	(srep--geometric-sequence-p '()))
      (expect '(srep--geometric-sequence-nth (2.0 1.0)) ; ?項
	...
	(srep--geometric-sequence-p '(1 2 3)))
      ...
      (expect '(1234 "1234" 0 "1234hoge") ; 先頭に正?
	(srep-ana:integer "1234hoge"))
      (expect '(1234 "+1234" 0 "+1234hoge") ; 先頭に符?付き正?
	(srep-ana:integer "+1234hoge"))
      (expect '(-1234 "-1234" 0 "-1234hoge") ; 先頭に負?
	(srep-ana:integer "-1234hoge"))
      (expect '(1234 "1234" 4 "hoge1234hoge") ; 途中に正?
	(srep-ana:integer "hoge1234hoge"))
      (expect '(1234 "+1234" 4 "hoge+1234hoge") ; 途中に符?付き正?
	(srep-ana:integer "hoge+1234hoge"))
      (expect '(-1234 "-1234" 4 "hoge-1234hoge") ; 途中に負?
	(srep-ana:integer "hoge-1234hoge"))
      (expect '(1234 "1234" 4 "hoge1234") ; 最後に正?
	(srep-ana:integer "hoge1234"))
      (expect '(1234 "+1234" 4 "hoge+1234") ; 途中に符?付き正?
	(srep-ana:integer "hoge+1234"))
      (expect '(-1234 "-1234" 4 "hoge-1234") ; 途中に負?
	(srep-ana:integer "hoge-1234"))
      (expect nil ; ?字が含まれない
	...
	(srep-rep:integer 1234 "0"))
      (expect "-1234" ; 負?
	(srep-rep:integer -1234 "0"))
      (expect "+1234" ; 正?、サンプルに符?付き (+)
	(srep-rep:integer 1234 "+1"))
      (expect "1234" ; 正?、サンプルに符?付き (-)
	(srep-rep:integer 1234 "-1"))
      (expect "-1234" ; 負?、サンプルに符?付き (+)
	(srep-rep:integer -1234 "+1"))
      (expect "-1234" ; 負?、サンプルに符?付き (-)
	(srep-rep:integer -1234 "-1"))
      (expect "01234" ; 正?、サンプルに桁指定付き
	(srep-rep:integer 1234 "00001"))
      (expect "-01234" ; 負?、サンプルに桁指定付き
	(srep-rep:integer -1234 "00001"))
      (expect "+01234" ; 正?、サンプルに符? (+)、桁指定付き
	(srep-rep:integer 1234 "+00001"))
      (expect "01234" ; 正?、サンプルに符? (-)、桁指定付き
	(srep-rep:integer 1234 "-00001"))
      (expect "-01234" ; 負?、サンプルに符? (+)、桁指定付き
	(srep-rep:integer -1234 "+00001"))
      (expect "-01234" ; 負?、サンプルに符? (-)、桁指定付き
	(srep-rep:integer -1234 "-00001")))))
